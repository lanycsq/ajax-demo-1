...
var server = http.createServer(function (request, response) {
    ...
    if (path === '/') {
        ...
        string = string.replace("{{page1}}", `<ul id="paging">${result}</ul>`)
        ...
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/javascript;charset=utf-8')
        const string = fs.readFileSync('public/ajax.js')
        response.write(string);
        response.end()
    } else if (path === '/ajax.html') {
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/html;charset=utf-8')
        const string = fs.readFileSync('public/ajax.html')
        response.write(string);
        response.end()
    } else if (path === '/ajax.xml') {
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/xml;charset=utf-8')
        const string = fs.readFileSync('public/ajax.xml')
        response.write(string);
        response.end()
    } else if (path === '/ajax.json') {
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/json;charset=utf-8')
        const string = fs.readFileSync('public/ajax.json')
        response.write(string);
        response.end()
    } else if (path === '/page2') {
        response.statusCode = 200
